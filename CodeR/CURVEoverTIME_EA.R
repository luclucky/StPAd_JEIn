...
pLoT =
	...
	scale_x_continuous("\nTime", breaks = seq(0,50,50), limits = c(0,77.5), sec.axis = sec_axis(~ . + 10, name = 'Climatic Event Scenario\n')) +
...
pLoT2 =
	...
	facet_grid(AF ~ DS) +
	geom_text_repel(data = subset(DF_ID, variable == "t 75"), aes(label = `LS`), box.padding = .1, min.segment.length = Inf, nudge_x = 3.75, nudge_y = 0, direction = "y", size = 3.75) +
	theme(strip.text = element_text(size=10), strip.background = element_blank(), panel.border = element_rect(colour = NA, fill = NA))

ggsave(filename=paste("/home/lucas/Desktop/PhD/STRESSOR/Documents/PUB_3rd/R_GRAPHs/GridEA_new.jpg", sep=""), pLoT2, width = 20, height = 12.5, units = "cm")
...
load('/home/lucas/Desktop/PhD/STRESSOR/Documents/PUB_3rd/R_DATA/EA_iA_new.Rda')
...
ggsave(filename=paste("/home/lucas/Desktop/PhD/STRESSOR/Documents/PUB_3rd/R_GRAPHs/GridEA_iA_new.jpg", sep=""), pLoT2, width = 20, height = 13.333, units = "cm")